...
const LeaveApplications = () => {
  ...
  const fetchLeaveApplications = (from, to, employeeName = "") => {
    axios
      .post("http://localhost:4001/getLeaveRequestsAll", {
        fromDate: from,
        toDate: to,
      })
      .then((response) => {
        ...
        filteredData = filteredData.filter(
          (leave) =>
            moment(leave.startDate).isBetween(from, to, undefined, "[]") ||
            moment(leave.endDate).isBetween(from, to, undefined, "[]")
        );

        setLeaveApplications(filteredData); // Set the filtered data
      })
      .catch((error) => {
  ...
  const handleAccept = (id, reason, remarks) => {
    axios
      .put("http://localhost:4001/updateLeaveStatus", {
        leaveId: id,
        remarks,
        status: "Accept",
      })
  ...
  // Function to fetch all employees from the leave applications
  const fetchEmployees = () => {
    // You don't need to make an additional API call for employees, just extract unique employee names
    const employeeNames = [
      ...new Set(leaveApplications.map((leave) => leave.employeeName)),
    ];
    ...
    >
      ...
      <div style={{ marginTop: "80px" }}>
        {leaveApplications.length === 0 ? (
          <p>
            No leave applications available for the selected date range and
            employee.
          </p>
        ) : (
