    ...
    return (
      <React.Fragment key={dateIndex}>
        {reports.map((reportItem) => {
          const reportDate = moment(dateItem.reportDate, "DD-MMM-YYYY").format("YYYY-MM-DD");
          ...
          return reportDetails.map((detailItem, detailIndex) => {
            ...
            return (
              <tr key={detailIndex} className="bg-white border-b hover:bg-gray-50">
                {isFirstDetail && (
                  <>
                    <td className="px-6 py-4 border" rowSpan={reportDetails.length}>
                      {serialNo++}
                    </td>
                    <td className="px-6 py-4 border" rowSpan={reportDetails.length}>
                      {reportDate}
                ...
                <td className="px-6 py-4 border">{detailItem.projectName || "N/A"}</td>
                <td className="px-6 py-4 border">
                  {subCategories.map((subCat, subCatIndex) => (
                    <div key={subCatIndex} className="border-b last:border-b-0">{subCat.subCategoryName}</div>
                  ))}
                </td>
                <td className="px-6 py-4 border">
                  {subCategories.map((subCat, subCatIndex) => (
                    <div key={subCatIndex} className="border-b last:border-b-0">{subCat.report || "No report available."}</div>
                  ))}
                </td>
              </tr>
            );
    ...
    return (
      <React.Fragment key={index}>
        {reports.map((reportItem, reportIndex) => {
          const reportDate = moment(employeeItem.reportDate, "DD-MMM-YYYY").format("YYYY-MM-DD");
          const subCategories = reportItem.subCategory;

          return (
            <tr key={reportIndex} className="bg-white border-b hover:bg-gray-50">
              {reportIndex === 0 && (
                <>
                  <td className="px-6 py-4 border" rowSpan={reports.length}>
                    {index + 1}
                  </td>
                  <td className="px-6 py-4 border" rowSpan={reports.length}>
                    {reportDate}
              ...
              <td className="px-6 py-4 border">{reportItem.projectName || "N/A"}</td>
              <td className="px-6 py-4 border">
                {subCategories.map((subCat, subCatIndex) => (
                  <div key={subCatIndex} className="border-b last:border-b-0">{subCat.subCategoryName}</div>
                ))}
      ...
      <div className="flex justify-between items-center my-5">
        ...
        <div>
          ...
          <button
            ...
            className={`px-4 py-2 text-white bg-black rounded-lg ${
              page * maxDisplayCount >= totalRecords ? "opacity-50 cursor-not-allowed" : ""
            }`}
          >
            Next
          </button>
